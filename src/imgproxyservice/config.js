
require('dotenv').config();

const DATABASE_PORT = process.env.PGSQL_SERVICE_PORT;
const PGSQL_SERVICE_API_ENDPOINT = `${process.env.PGSQL_SERVICE}:${process.env.PGSQL_SERVICE_PORT}`;
const IMGPROXY_PORT= process.env.IMGPROXY_SERVICE_PORT;
const IMGPROXY_SERVICE_PORT = process.env.IMGPROXY_SERVICE_PORT;
const IMGPROXY_SERVICE = process.env.IMGPROXY_SERVICE;
const IMGPROXY_SERVICE_API_ENDPOINT= `0.0.0.0:${IMGPROXY_SERVICE_PORT}`;
const CADDY_PORT = process.env.CADDY_SERVER_PORT;
const CADDY_SERVICE = process.env.CADDY_SERVER_SERVICE;
const FACEAPI_SERVICE = process.env.FACEAPI_SERVICE;
const FACEAPI_SERVICE_PORT = process.env.FACEAPI_SERVICE_PORT;
const FACEAPI_SERVICE_API_ENDPOINT = `${FACEAPI_SERVICE}:${FACEAPI_SERVICE_PORT}`;

module.exports = {
    DATABASE_PORT,
    PGSQL_SERVICE_API_ENDPOINT,
    IMGPROXY_PORT,
    IMGPROXY_SERVICE_PORT,
    IMGPROXY_SERVICE_API_ENDPOINT,
    IMGPROXY_SERVICE,
    CADDY_PORT,
    CADDY_SERVICE,
    FACEAPI_SERVICE_API_ENDPOINT
}