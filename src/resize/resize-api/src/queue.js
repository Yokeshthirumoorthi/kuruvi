...
const {RESIZE_QUEUE_ENDPOINT} = require('./common/config');
const {resizeImage} = require('./services');
const QUEUE_CONNECTION_STRING = `amqp://${RESIZE_QUEUE_ENDPOINT}`;

function runServices(message, sendAckToQueue) {
    console.log(" [x] Received %s", message);
    resizeImage(message, sendAckToQueue)
}
 ...
 */
function runResizeAlbumUsingQueue(call, callback) {
  const requestMessage = call.request; 
  var msgs = requestMessage.photos.map(
    photoName => {
        return {
            albumName: requestMessage.albumName,
            photoName: photoName
        }
    }
  );
  ...
  sendToQueue(requestMessage.albumName, [...msgs, endMessage]);
  consumeQueue(requestMessage.albumName, callback);
}

module.exports = {runResizeAlbumUsingQueue}