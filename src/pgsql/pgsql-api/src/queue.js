...
const {PGSQL_QUEUE_ENDPOINT} = require('./common/config');
const services = require('./services');
const QUEUE_CONNECTION_STRING = `amqp://${PGSQL_QUEUE_ENDPOINT}`;

function runServices(message, sendAckToQueue) {
    console.log(" [x] Received %s", message);
    services.organizeAlbum(message, sendAckToQueue)
}
 ...
 */
function runOrganizeDataUsingQueue(call, callback) {
    console.log("Inside save organize folder  using queue",call.request);
    const requestMessage = call.request;
    const albumName = requestMessage.albumName;
    var msgs = [{albumName: albumName}];
    const endMessage = {
        albumName: ''
    };
...
module.exports = {runOrganizeDataUsingQueue}
